...
class TodoService{
  ...
  createTodo = async (title: string, description: string, dueDate: Date, categoryId: number): Promise<number | undefined> => {
    const cat = await catRepository.findOneBy({ id: categoryId });
    ...
    const todo = await todoRepository.save(
      todoRepository.create({ title, description, due_date: dueDate, category: cat }) 
    );
  ...
  updateTodoItem = async(item:Partial<Todo>, categoryId?:number): Promise<number|undefined>=>{
    const [cat,todo] = await Promise.all([catRepository.findOneBy({id:categoryId}),todoRepository.findOneBy({id:item.id})]);
    if(!cat || !todo){
      throw new NotFoundError("category or todo not found");
    }
    await todoRepository.save({
      ...todo,...item,category:{...cat}
    });
    return item.id;
  };
