...
class TodoController{
  ...
  createTodo:MiddlewareFunction<Promise<number|undefined>> = async (req,_res,_next)=>{
    const { title,description,dueDate, categoryId } = req.body as unknown as any;
    return todoService.createTodo(title,description,dueDate, categoryId );
  };
  ...
  updateTodo:MiddlewareFunction<Promise<number|undefined>> = async(req,_res,_next)=>{
    return todoService.updateTodoItem({title:req.body.title,
      description: req.body.description,
      due_date: req.body.dueDate,
      id:req.params.todoId as unknown as number},req.body.categoryId);
  };
